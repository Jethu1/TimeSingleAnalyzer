...
/**
 * Created by jet on 2017/8/11.
 */
public class TimeSingleFilter extends Filter {
    ...
    public TimeSingleFilter(String field, String[] words, int begin, int end) {
        this.field = field;
        this.begin = begin;
        this.end = end;
        this.words = words;
    ...
    public DocIdSet getDocIdSet(AtomicReaderContext context, Bits acceptDocs) throws IOException {
        ...
        //获取每个词的文档号和词语集合
            ...
            DocPosition[] docPos1 = new DocPosition[words.length];
            for (int i = 0; i < words.length; i++) {
                ...
                docPos1[i] = new DocPosition(words[i], termsEnum.docsAndPositions(acceptDocs, null));
            }//一个词对应的单字的文档号集合
        ...
        while (it.hasNext()) {
            ...
            //获取当前docID
            int docID = it.next();
            for (int i = 0; i < docPos1.length; i++) {
                docPos1[i].skipTo(docID);
            }
            boolean flag = false;
            //计算前、后的字是否依次相邻，循环是判断一篇文档是否有多个长词满足条件
            List<TimeItem> timeItems = new ArrayList<TimeItem>();
            do {
                if (SearchUtils.findFollowPosition(docPos1, 0, Integer.MAX_VALUE)) {
                    if(begin<=docPos1[0].getDocsEnum().startOffset()&&docPos1[docPos.length - 1].getDocsEnum().endOffset()<=end){
//                      System.out.println("第一个词位置： "+docPos[0].position()+"最后一个词位置： "+docPos[docPos.length-1].position());
                        flag = true;
                        TimeItem item = new TimeItem();
                        item.setBegin(docPos1[0].getDocsEnum().startOffset());
                        item.setEnd(docPos1[docPos.length - 1].getDocsEnum().endOffset());
                        item.setText(value);
                        timeItems.add(item);
                    }
                }
            }while(docPos1[0].nextPosition());
